...
int ledPin2=12;
long delay_time=3;
long delay_times= 1000; //set 20ms for 50Hz, 100ms for 10Hz
...
void setup() {
 ...
 DDRB = DDRB | B10000000; //pinMode(ledPin, OUTPUT);
 ...
 PORTB |= B10000000;//digitalWrite(ledPin, HIGH);
 delay(10);
 PORTB &= B01111111; //digitalWrite(ledPin, LOW);
 ...
 for(int i=0; i<length ; i++)
  {
    ...
    //consider the last bit of transmission
      ...
      else if(values.charAt(i) == '1') 
      {
        ...
        //delay(delay_time);
        delayMicroseconds(delay_times);
        PORTB &= B01111111;//digitalWrite(ledPin, LOW); // a long sequence of zero for indicating the end of tx
      }
    }
  }
